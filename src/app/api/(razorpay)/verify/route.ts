...
const generatedSignature = (razorpayOrderId: string, razorpayPaymentId: string, secret_key: string) => {
    return crypto
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!secret_key) {
            return NextResponse.json({
                success: false,
                message: "Please configure your razorpay secret key in the environment"
            }, {
                status: 404
            })
        }
        ...
        const signature = generatedSignature(razorpayOrderId, razorpayPaymentId, secret_key);
        ...
        if (signature !== razorpaySignature) {
            ...
            return NextResponse.json(
                { success: false, message: 'Payment verification failed, Invalid payment signature' },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json(
            { success: true, message: 'Payment verified successfully', details: paymentOrder },
            { status: 200 }
        );
