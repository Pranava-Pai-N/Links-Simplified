    ...
    try {
        ...
        if (link.user.emailId !== session.user.email) {
            return NextResponse.json(
                {
                    success: false,
                    message: "Only authorized link creators hold the permissions required to update this resource",
                },
                { status: 403 },
            );
        }
        ...
        console.log("Error changing the status of the URL. Please try again later", error);
